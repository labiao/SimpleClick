...
class InteractiveDemoApp(ttk.Frame):
    def __init__(self, master, args, model):
        ...
        master.bind('<Key-D>', self._set_next_image)
        master.bind('<Key-d>', self._set_next_image)
        #master.bind('<Key-X>', self._set_forward_image)
        master.bind('<Key-R>',  lambda event:self.controller.undo_click())
        master.bind('<Key-P>', lambda event: self._reset_last_object)
        #master.bind('<Key-S>', self._save_mask_callback)
    ...
    def _load_image_callback(self): ####################图像的加载
        self.menubar.focus_set()
        if self._check_entry(self):
            self.filenames = filedialog.askopenfilenames(parent=self.master, filetypes=[
                ("Images", "*.tif *.jpg *.JPG *.jpeg *.png *.bmp *.tiff"),
                ("All files", "*.*"),
            ], title="Chose an image")
            print(12131313,self.filenames)
            namew=os.listdir(savepath)
            #print(namew)
            self.filenames1=list(self.filenames)
            #print(14444,self.filenames1)
            for name in namew:
                for fname in self.filenames1:
                    #print(13131414,name.split('.')[0])
                    if str(name.split('.')[0]) in fname.split('//')[-1]:
                        self.filenames1.remove(fname)
            self.filenames=tuple(self.filenames1)
            print(252568,self.filenames)
